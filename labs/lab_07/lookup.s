...
main:
    ...
    #addi  $t2, $t0, -1			#$t2 = x - 1
    #mul   $t2, $t2, 24          #$t2 = (x-1)*24
    #addi  $t3, $t1, -1			#$t3 = (y-1)
    #add  $t3, $t3, $t2			#$t3 = (x-1)*24 + (y-1)

    mul   $t2, $t0, 24
    add   $t3, $t2, $t1	

    #$t3 is our index so far 
    li	$t4, 4
    mul	$t3, $t3, $t4	
    la	$t4, array	 
    add	$t4, $t3, $t4 #final address = &array + (index*4 bytes)
    
    lw	$t4, ($t4)	#load (numbers + t4 bytes) in t4  
